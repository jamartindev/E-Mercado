...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function appendComments() {
        ...
        for (let i = 0; i < comments.length; i++) {
            ...
            carouselComments.push(
              () => `<div id="comentariosGenerales" class="container-fluid">
                        <div class="mx-auto" style="display: flex; flex-direction: column; width: 50vh; align-items: flex-end">
                            <h2>Este producto no tiene comentarios.</h2><br>
                            <h2>Sé el primero en dejar tu opinión!</h2><br>
                        </div>
                    </div>`
            );
        }
        ...
        function disableButtons() {
            if (counter <= 0 || comments.length === 0) {
                btnCarouselPrev.disabled = true;
            } else {
                btnCarouselPrev.disabled = false;
            };

            if (counter === (comments.length-1) || comments.length === 0) {
